...
4.  [x] Standalone runner (Storybook test-runner mode) - Second capture path built on @storybook/test-runner, shipped as both a CLI (`qlip-serve-and-test`) and a library (`@qoretechnologies/qlip/test-runner`). Same manifest, same upload protocol, same review UI. Design: `design/RUNNER.md`. Tracker: `.tasks/STANDALONE_RUNNER.md`.
5.  [x] Vitest plugin memory-safe config verified - on 2026-05-25, qorus-ide (90 files / 643 tests / Vitest 2.1.9 / Storybook 8.5) completed cleanly in 7 min with 7.9 GB peak RSS using `pool: 'forks' + maxForks: 3 + isolate: true`. The 2026-05-23 conclusion that the runner was required for qorus-ide-class workloads is superseded; both paths are now first-class. Documented in `design/INTEGRATION_PATHS.md` + `README.md`.
6.  [x] Append-only manifest fragments (issues #25 + #26) - captures were lost when several story files shared a browser context under `isolate: false`: qlip re-serialised the whole manifest per capture into one file per context, and the write that landed last won. Now one append-only fragment per capture, tombstones for the retry-mask prune, plus a post-merge audit (`capture-report.json`, orphan-PNG warning, `failOnPartialBuild`) so a partial build can never be silent again. Design: `design/MANIFEST_FRAGMENTS.md`. Tracker: `.tasks/FRAGMENT_WRITE_RACE.md`.
