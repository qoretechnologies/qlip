...
4.  [ ] Standalone runner (Storybook test-runner mode) - Second capture path that decouples qlip from the consumer's Vitest version. Built on @storybook/test-runner (same approach Chromatic uses). Sits alongside the existing Vitest plugin; same manifest, same upload protocol, same review UI. Design: `design/RUNNER.md`. Tracker + acceptance checklist: `.tasks/STANDALONE_RUNNER.md`. ~5 days; no qlip-server or qlip-ui changes.
